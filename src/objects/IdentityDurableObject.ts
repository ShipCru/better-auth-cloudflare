...
export class IdentityDurableObject<Env extends IdentityDurableObjectEnv = IdentityDurableObjectEnv> extends DurableObject<Env> {
  ...
  async reserve(emailHash: string): Promise<ReserveResult> {
    ...
    const existing = runSql(sql, `SELECT * FROM identity WHERE email_hash = ?`, emailHash)
      .toArray()[0] as RawIdentity | undefined;
  ...
  async commit(emailHash: string, reservationId: string, principalId: string): Promise<CommitResult> {
    const sql = this.ctx.storage.sql;
    const row = runSql(sql, `SELECT * FROM identity WHERE email_hash = ?`, emailHash).toArray()[0] as
      | RawIdentity
      | undefined;
